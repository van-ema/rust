...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    pub fn codegen_rvalue_operand(
        ...
        match *rvalue {
            ...
            mir::Rvalue::CopyForDeref(place) => {
                if std::env::var("DEBUG").is_ok() {
                    println!("[codegen_rvalue_operand] Rvalue::CopyForDeref {:?}", rvalue);
                }
                self.codegen_operand(bx, &mir::Operand::Copy(place))
            }
            mir::Rvalue::RawPtr(mutability, place) => {
                if std::env::var("DEBUG").is_ok() {
                    println!("[codegen_rvalue_operand] Rvalue::RawPtr {:?}", rvalue);
                }
                let mk_ptr =
                    move |tcx: TyCtxt<'tcx>, ty: Ty<'tcx>| Ty::new_ptr(tcx, ty, mutability);
                self.codegen_place_to_pointer_with_rawptr_metadata(bx, place, mk_ptr)
            }
    ...
    fn codegen_place_to_pointer_with_rawptr_metadata(
        &mut self,
        bx: &mut Bx,
        place: mir::Place<'tcx>,
        mk_ptr_ty: impl FnOnce(TyCtxt<'tcx>, Ty<'tcx>) -> Ty<'tcx>,
        ...
        println!("[codegen_rvalue_rawptr] dest={:?}", cg_place.val.llval);
